...
- **This fork's root README is the author's implementation catalog.** After every commit authored by Nguyen Ba Huan, update `README.md` so it lists only that author's implementations and what the new commit shipped; keep `README.zh.md` aligned and re-record the pair ([policy](.agents/notes/implemented/process/2026-08-17-fork-readme-implementation-catalog.md)).
